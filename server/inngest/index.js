  ...
  async ({ event, step }) => {
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      await sendEmail({
        ...
        body: `<div style="max-width:600px;">
    <h2>Hi Admin, 👋</h2>
    <p style="font-size:16px;">A leave application is pending your action.</p>
    <p style="font-size:16px;"><strong>Employee:</strong> ${employee.firstName} ${employee.lastName} (${employee.department})</p>
    <p style="font-size:16px;"><strong>Leave Type:</strong> ${
      leaveApplication.type === "HALF_DAY"
        ? `Half Day (${leaveApplication.halfDayPeriod === "FIRST" ? "Morning" : "Afternoon"})`
        : leaveApplication.type
    }</p>
    <p style="font-size:16px;"><strong>Date:</strong> ${leaveApplication.startDate?.toLocaleDateString()}</p>
    <p style="font-size:16px;">Please log in to take action on this application.</p>
    <br/>
